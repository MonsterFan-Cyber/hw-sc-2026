...
## 轨迹线法生成 NFP
### 代码贡献者(mxlkevin, MonsterFan-Cyber)
### 轨迹线生成
...
## 周期模拟退火组合优化
### 代码贡献者(MonsterFan-Cyber, yuhaozhou2000)
...
交换两个多边形的中心位置，选择策略随进度变化：前期选距原点最远的多边形；中期按距离加权随机选取；后期按距离中位数筛选 + 尺寸匹配（面积比 > 0.6）。交换后若碰撞，对双方依次沿 MTV 方向推开修正（`+mtv * 1.001`），先修正 ia 再检查 ib，两次修正后均无碰撞才按 SA 准则接受。
...
- **多边形膨胀容差**（`expandPolygon`）：对输入多边形做微小外扩（4e-5），采用斜接连接（miter join）并限制斜接比，避免因浮点误差导致的假阴性碰撞，可能会导致多边形顶点数略微提高。
- **网格吸附** (`snapGrid`): 将扩展后的多边形进行 `1e-5` 的网格吸附，这一步可以改善极小/极大斜对后续计算和判断率导致的误判。 
- **凸包近似**：对顶点数 > 50 且包围盒面积占比小于最大包围盒面积 `10%` 的小而复杂多边形，用凸包替代以加速 NFP 计算
